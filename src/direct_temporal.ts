...
export class DirectTemporalProcessHandler {
    ...
    private temporalSteps: TemporalStep[] = [];
    ...
    private start() {
        ...
        this.showMessage('Starting direct temporal process...', 'process-info');
    ...
    stop() {
        ...
        this.showMessage('Direct temporal process stopped and reset.', 'error');
    }
    ...
    private executeStep() {
        if (this.currentStep >= FUNCTIONS.length) {
            ...
            this.showMessage(`All steps completed in ${(elapsedTime / 1000).toFixed(2)}s!`, 'success');
            this.isRunning = false;
            if (this.interval) {
                clearInterval(this.interval);
                this.interval = null;
        ...
        while (this.currentStep < FUNCTIONS.length) {
            ...
            if (func.name === 'cupAddMilk' && !document.getElementById('toggleMilk')?.classList.contains('active')) {
                this.recordTemporalStep(func.name);
    ...
    private recordTemporalStep(functionName: string) {
        const currentTime = Date.now() - this.startTime;
        this.temporalSteps.push({
            functionName,
        ...
        if (this.temporalSteps.length > 1) {
            const prevStep = this.temporalSteps[this.temporalSteps.length - 2];
            prevStep.duration = currentTime - prevStep.timestamp;
        }
    }
    ...
    getTemporalSteps(): TemporalStep[] {
        return this.temporalSteps;
    }
}