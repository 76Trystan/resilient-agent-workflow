...
export class AgenticTemporalProcessHandler {
  ...
  private async start() {
    ...
    this.showMessage('Starting agentic temporal workflow...', 'process-info');
  ...
  private displayAgentMessage(analysis: string, action: string, confidence: number) {
    const llmChat = document.getElementById('llmChat');
    const llmChatContent = document.getElementById('llmChatContent');

    if (!llmChat || !llmChatContent) return;
    ...
    llmChatContent.innerHTML = message;
    llmChat.classList.add('active');
  }

  private clearAgentMessage() {
    const llmChat = document.getElementById('llmChat');
    const llmChatContent = document.getElementById('llmChatContent');
